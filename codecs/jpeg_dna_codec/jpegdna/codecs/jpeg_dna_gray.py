...
from jpegdna.coders.categorycoder import (
    NonDecodableCategory,
    find_category_dc,
    count_run_cat,
)
...
class JPEGDNAGray(AbstractCoder):
    ...
    def __init__(
        ...
        self.codebook = load_codebook_matrix(
            Path(jpegdna.__path__[0] + "/data/codebook.pkl")
        )
        self.dct = DCT()
        self.zigzag = ZigZag(verbose=False)
        self.total_runlength_nts, self.freq_dc, self.freq_ac, self.m, self.n = (
            None,
            None,
            None,
            None,
        ...
        self.remain = ""
        if self.formatting:
            self.formatter = JpegDNAFormatter(
                self.alpha,
                "gray",
                None,
                primer=self.primer,
    ...
    def set_state(self, *args, case=None):
        ...
        if case == "encode":
            ...
            if self.alpha < min_alpha:
                raise ValueError(
                    f"Invalid alpha value, minimal possible value for this image: {min_alpha}"
                )
            if len(args) < 2:
                raise ValueError
            if args[1] == "from_img":
                ...
                raise ValueError(
                    "No frequency specified, pick between {'from_img'|'from_file'|'default'}"
                )
        elif case == "decode":
            if len(args) < 3:
                raise ValueError
        ...
        for i in range(nb_row_blocks):
            for j in range(nb_col_blocks):
                ...
                if channel_type == "luma":
                    ...
                    raise ValueError(
                        "Wrong channel type, either pick 'luma' or 'chroma'"
                    )
    ...
    def set_frequencies_from_img(self, inp):
        ...
        for i in range(nb_row_blocks):
            ...
            for j in range(nb_col_blocks):
                ...
                (count_run_end, run_cat_count, count_run16) = count_run_cat(
                    seq_coeff, self.lut
                )
    ...
    def encode(self, inp, apply_dct=True):
        ...
        self.zigzag.verbose = (
            self.verbose and self.verbosity >= ZIG_ZAG_VERBOSITY_THRESHOLD
        )
        ...
        if self.verbose and self.verbosity >= HUFFMAN_VERBOSITY_THRESHOLD:
            ...
            print(f"Frequencies:\n{self.freq_dc}")
        self.huffman_dc_coder = HuffmanCoder(
            range(11),
            self.freq_dc,
            3,
        ...
        if self.EOB_SHIFT:
            ...
            dummy_freq_ac_idx = np.argpartition(self.freq_ac, -EOB_SHIFT_IDX)[
                -EOB_SHIFT_IDX:
            ]
            ...
            if val != 1:
                ...
                self.freq_ac[-1] = 1
        self.huffman_ac_coder = HuffmanCoder(
            range(162),
            self.freq_ac,
            3,
            verbose=(self.verbose and self.verbosity >= HUFFMAN_VERBOSITY_THRESHOLD),
        ...
        self.dc_coeff_coder = DCCoefficientCoder(
            self.huffman_dc_coder.dic,
            self.codebook,
            verbose=(
                self.verbose and self.verbosity >= VALUE_CODER_VERBOSITY_THRESHOLD
            ),
        )
        self.ac_coeff_coder = ACCoefficientCoder(
            self.huffman_ac_coder.dic,
            self.lut,
            self.codebook,
            verbose=(
                self.verbose and self.verbosity >= VALUE_CODER_VERBOSITY_THRESHOLD
        ...
        for i in range(nb_row_blocks):
            ...
            for j in range(nb_col_blocks):
                ...
                (code_dc, count_cat_len) = self.dc_coeff_coder.full_encode(
                    diff, gold_code_dc
                )
                (code_ac, count_runcat_len) = self.ac_coeff_coder.full_encode(
                    seq_coeff, gold_code_ac
                )
                # category length
    ...
    def decode(self, code, apply_dct=True):
        ...
        for i in range(nb_row_blocks):
            ...
            for j in range(nb_col_blocks):
                    ...
                    if code == "":
                        ...
                        while len(seq_coeff) < 64:
                            if code == "":
                                ...
                                (
                                    coeff,
                                    (num_zeros, num_bits, end_of_block),
                                ) = self.ac_coeff_coder.full_decode(code)
                                if end_of_block:
                                    for _ in range(64 - len(seq_coeff)):
                                        seq_coeff.append(0)
                ...
                if self.verbose and self.verbosity >= BLOCK_VERBOSITY_THRESHOLD:
                    print(f"--------------------\nDecoded block ({i},{j}):\n{block}")
        jpeg_decoded = np.clip(jpeg_decoded[: self.n, : self.m], 0, 255).astype(
            np.uint8
        )
