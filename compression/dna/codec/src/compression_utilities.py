...
def bits_to_nucleotides(bitstream):
    """
    Converts a bitstream into a nucleotide string.
    
    Parameters:
    bitstream: Bitstream to convert.
    ...
    return "".join(
            [{'00': 'A', '01': 'C', '10': 'G', '11': 'T'}[bitstream[i:i+2]] for i in range(0, len(bitstream), 2)])
    
def bits(x):
    return bin(int.from_bytes(bytes(str(x).encode('utf-8')), byteorder=sys.byteorder)).lstrip('0b')

def pack_tensor(threshold, y_shape, z_strings, bytes_length=2) -> str:
    """
    ...
    # TODO: translate everything as nucelotides string
    return b""


  
def pack_tensor_bytes(threshold, x_shape, y_shape, z_shape, z_string, *y_strings,  bytes_length = 2, adaptive = False):
    """
...
def compute_optimal_threshold(model, z_strings, y_shape, pc, delta_t = 0.01, breakpt = 50, verbose = 1):
    """
